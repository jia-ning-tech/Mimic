...
- Outputs:
  - outputs/metrics_test_<model>[ _<method>].csv
  - outputs/metrics_test.csv  (append/update by model+method)
  - outputs/model_auc_test.csv
  - outputs/thresholds_<model>[ _<method>].csv
  - outputs/roc_test_<model>[ _<method>].png
  - outputs/pr_test_<model>[ _<method>].png
  - outputs/calibration_test_<model>[ _<method>].png
"""
...
# ---- common utils (no behavior change) ----
...
def append_or_update_summary(summary_csv: Path, metrics_row: Dict[str, Any], keys: List[str]):
    ...
    Ensure 'model' column exists to avoid KeyError seen earlier.
    """
...
def main(argv=None):
    ...
    # Default threshold 0.5 for point metrics table (consistent with earlier runs)
    ...
    metrics_csv = outputs_dir / f"metrics_test_{args.model}{suffix}.csv"
    metrics_row = {
        "model": args.model,
        "method": args.method,
        "n_test": int(len(y)),
    ...
    thr_csv = outputs_dir / f"thresholds_{args.model}{suffix}.csv"
    ...
    roc_png = outputs_dir / f"roc_test_{args.model}{suffix}.png"
    pr_png  = outputs_dir / f"pr_test_{args.model}{suffix}.png"
    calib_png = outputs_dir / f"calibration_test_{args.model}{suffix}.png"
